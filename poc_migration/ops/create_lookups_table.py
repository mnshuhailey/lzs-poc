...
from poc_migration.resources import lzs_poc_migration_resource  # Updated to target resource

def read_sql_file(file_path):
    with open(file_path, 'r') as file:
        return file.read()

@op(out=Out(Nothing), required_resource_keys={"lzs_poc_migration"})
def create_lookups_table(context):
    base_dir = os.path.dirname(os.path.realpath(__file__))
    sql_file_path = os.path.join(base_dir, '../sql/create_lookups_table.sql')
